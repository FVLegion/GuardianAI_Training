...
def setup_ui():
    ...
    with st.sidebar:
        ...
        if enable_email:
            ...
            sender = st.text_input("Your Email Address", placeholder="your@email.com")
            password = st.text_input("Email Password", type="password", help="Generate an app password from your Google account")
        else:
    ...
    if st.session_state.run:
        try:
            ...
            if st.session_state.processor is None:
                with st.spinner("🔄 Loading AI models..."):                    # Model paths
                    pose_model_path = "inference/yolo11n-pose.pt"
                    # Use the model that matches the config file
                    action_model_path = "extracted_guardian_ai_artifact/best_bilstm_github_7edd2cb791f44aea8c55e1f12c3f4c6c.pth"
                      # Load models
                    pose_model = YOLO(pose_model_path)
                    action_classes = ["Falling", "No Action", "Waving"]
                    
                    # Load model parameters dynamically from config file
                    model_config = load_model_config()
                    input_size = model_config["input_size"]
                    hidden_size = model_config["hidden_size"]
                    num_layers = model_config["num_layers"]
                    num_classes = len(action_classes)  # Override with actual class count
...
def load_model_config(config_path="extracted_guardian_ai_artifact/model_config_7edd2cb7.json"):
    """
    ...
    Returns:
        dict: Model architecture parameters
    """
    try:
        ...
        if not os.path.exists(config_path):
            print(f"⚠️ Model config file not found at {config_path}, using default parameters")
            ...
            return {
                ...
                "num_layers": 3,
                "num_classes": 3,
                "dropout_rate": 0.4222015160454323,
                "use_layer_norm": True,
                "attention_dropout": 0.060093554632455165
            }
        ...
        model_arch = config_data.get("model_architecture", {})
        
        # Convert string values to appropriate types
        params = {
            "input_size": int(model_arch.get("input_size", 34)),
            "hidden_size": int(model_arch.get("hidden_size", 128)),
            "num_layers": int(model_arch.get("num_layers", 3)),
            "num_classes": int(model_arch.get("num_classes", 3)),
            "dropout_rate": float(model_arch.get("dropout_rate", 0.4222015160454323)),
            "use_layer_norm": model_arch.get("use_layer_norm", "True").lower() == "true",
            "attention_dropout": float(model_arch.get("attention_dropout", 0.060093554632455165))
        }
        
        print(f"✅ Successfully loaded model config from {config_path}")
